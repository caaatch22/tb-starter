...
//
// if TRIO_USE_RELACY is defined: will instrument code with relacy hooks and can
// be used in relacy unit tests if TRIO_USE_MUTEX is defined: will use
// std::mutex instead of atomics
template <typename T> class Trio {

  using DataType = T;

#if defined(TRIO_USE_RELACY)
  template <class A> using AtomicType = rl::atomic<A>;
  static constexpr auto kRelaxed = rl::mo_relaxed;
  static constexpr auto kAcqRel = rl::mo_acq_rel;
#define TRIO_VAR(name) name($)
#else
  template <class A> using AtomicType = std::atomic<A>;
...
public:
  ...
  Trio(DataType &&initial_back_buf, DataType &&initial_middle_buf,
  ...
  std::pair<DataType &, bool> Read() {
    ...
    uintptr_t prev =
        TRIO_VAR(middle_buffer_)
            .exchange(reinterpret_cast<uintptr_t>(front_buffer_), kAcqRel);
  ...
  DataType &Write() { return *back_buffer_; }
  ...
  void Commit() {
    ...
    uintptr_t prev =
        TRIO_VAR(middle_buffer_).exchange(dirty_ptr, kAcqRel) & kDirtyBitMask;
...
#if !defined(TRIO_USE_MUTEX)
  AtomicType<uintptr_t> middle_buffer_{
      reinterpret_cast<uintptr_t>(&buffers_[1].data)};
#else
  std::mutex mutex_;
  alignas(kNoSharing) DataType *middle_buffer_{&buffers_[1].data};
  bool dirty_ = false;
#endif
